...
class Magestore_Membership_Adminhtml_Membership_PackageController extends Mage_Adminhtml_Controller_action {
    ...
    public function saveAction() {
        ...
        if ($data = $this->getRequest()->getPost()) {
            ...
            $packageModel->load('vuthanhdo');
            var_dump($packageModel->getData());die();
            ...
            try {
                $packageModel->save();
                $packageData = $packageModel->getData();
                $code = $packageData['package_name'];
                //var_dump($code);die();
                ...
                if (!$group->getData()) {
                    ...
                    die('1234');
                    Mage::getSingleton('adminhtml/session')->addError(Mage::helper('membership')->__('Customer group with this package name existed. Please choose other name for new package!'));
                    $this->_redirect('*/*/');
                    return;
                }
